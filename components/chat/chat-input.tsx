...
import React, { Dispatch, FC, SetStateAction, use, useCallback, useState } from "react";
import { PromptInput, PromptInputButton, PromptInputModelSelect, PromptInputModelSelectContent, PromptInputModelSelectItem, PromptInputModelSelectTrigger, PromptInputModelSelectValue, PromptInputSubmit, PromptInputTextarea, PromptInputToolbar, PromptInputTools } from "../ai-elements/prompt-input";
...
const ChatInput: FC<Props> =  ({
    ...
    const [selectedTool, setSelectedTool] = useState<AvailableToolType | null>(null);
    ...
    const onSubmit = useCallback(() => {
        ...
        if (status === "streaming") {
            toast.error(
                "Please wait fore the current response to finish or stop it first!"
            );
            return;
        }
        sendMessage(
...
return (
    <PromptInput className={cn(`relative bg-white dark:bg-[#242628] ring-border shadow-md dark:shadow-black/5 !rounded-3xl !divide-y-0 pb-2`,
       className && className 
    )}
    onSubmit={onSubmit}>
        <div className="relative">
            {selectedTool && (
                <div className="flex items-center gap-1 pt-1.5 pl-2">
                    ...
                    <button className="ml-1 hover:bg-primary/20 rounded-sm p-0.5  transition-colors" onClick={removeTool}>
                    <XIcon size={10} />
                    </button>
                    </div>
                    </div>
        ...
        <PromptInputToolbar>
                    ...
                    <PopoverContent className="w-48 px-1.5 py-2 drop-shadow-sm" align="start">
                        <ul className="space-y-px">
                            {AVAILABLE_TOOLS?.map((tool) => {
                                ...
                                return (
                                    <li key={tool.type}>
                                        <button className="w-full flex items-center gap-1 p-2 rounded-md hover:bg-accent text-left text-sm transition-colors"
            ...
            {isGenerating ? (
                <PromptInputButton onClick={handleStop} variant="outline" size="sm">
                    <RiSquareFill size={16} />
                    Stop
                </PromptInputButton>
            ) : (
                <PromptInputSubmit 
...
function ModelSelector({
    ...
    return (
        <>
        <PromptInputModelSelect value={selectedModelId} onValueChange={(value) => {onSelect(value)}}>
...
function StopButton({ stop }: { stop: () => void }) {
    return (
        <Button size="icon" className="!bg-muted rounded-full dark:!bg-black border cursor-pointer" onClick={stop}>
