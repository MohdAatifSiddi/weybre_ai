...
import React, { type ChangeEventHandler, Children, type ComponentProps, createContext, type FormEvent, type FormEventHandler, Fragment, type HTMLAttributes, type KeyboardEventHandler, type RefObject, use, useCallback, useContext, useEffect, useLayoutEffect, useMemo, useRef, useState } from "react";
...
export function PromptInputAttachment({
  ...
  return (
    <div className={cn("group relative h-14 w-14 rounded-md border", className)}
     key={data.id}
      {...props}>
...
export function PromptInputAttachments({
  ...
  return (
    <div aria-live="polite" className={cn("overflow-hidden transition-[height] duration-200 ease-out", className)}
...
export const PromptInputActionAddAttachments = ({
  ...
  return (
    <DropdownMenuItem {...props} onSelect={(e) => {
      e.preventDefault();
...
export const PromptInput = ({
  ...
  const handleSubmit: FormEventHandler<HTMLFormElement> = (event) => {
    ...
    const files: FileUIPart[] = items.map(({ id, ...item }) => item);
...
export const PromptInputTextarea = ({
  ...
  return (
    <Textarea className={cn("w-full resize-none rounded-none border-none p-3 shadow-none outline-none ring-0",
      "field-sizing-content bg-transparent dark:bg-transparent",
      "max-h-48 min-h-16",
      "focus-visible:ring-0", className
     )}
     name="message"
     onChange={(e) => {
      onChange?.(e);
...
export const PromptInputTools = ({
  ...
  <div className={cn("flex items-center gap-1", "[&_button:first-child]:rounded-bl-xl", className)}
  {...props}
  />
);
...
export const PromptInputButton = ({
  ...
  return (
    <Button className={cn("shrink-0 gap-1.5 rounded-lg", variant === "ghost" && "text-muted-foreground", newSize === "default" && "px-3", className)} size={newSize} type="button" variant={variant} {...props} />
  );
};
...
export const PromptInputActionMenuContent = (props: PromptInputActionMenuContentProps) => (
