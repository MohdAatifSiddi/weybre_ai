"use client"
import { RiScanLine, RiChatAiLine, RiBankCard2Line, RiSettings3Fill, RiSettings3Line } from "@remixicon/react";
...
const AppSidebar = ({...props}: React.ComponentProps<typeof Sidebar>) => {
  ...
  return (
    ...
    <Sidebar {...props} className="z-[99]" collapsible="icon">
      <SidebarHeader>
        <div className="w-full flex items-center justify-between">
          <Logo url="/home" />
          <SidebarTrigger className="-ms-9" />
        </div>
      ...
      <SidebarFooter>
        <hr className="border-border mx-2 -mt-px" />
        <NavUser isLoading={isPending} user={{name: user?.name || "",
          email: user?.email || "",
        }} 
