...
const PlanCard = React.memo(
    ({ plan, subscription, loading, error, isUpgrading, onUpgrade }: Props) => {
        ...
        const remainingGenerations = isCurrent ? subscription?.remainingGenerations : null;

        const percentUsed = generationsLimit && generationsLimit > 0 ? Math.min((generationsUsed / generationsLimit) * 100, 100) : 0;

        return (
            <div className="flex flex-col p-6 border-1">
...
export const FeatureRow = React.memo(({ features }: { features: readonly string[] }) => {
    return (
