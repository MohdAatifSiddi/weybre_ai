...
const Header = ({ title, showActions = false }: Props) => {
    ...
    return (
        <header className="fixed top-0 inset-0 z-[9] h-[40px] flex items-center px-2 md:px-8 py-1 bg-background/20 backdrop-blur-sm">
            ...
            {showActions && (
                <div className={cn("w-full flex items-center justify-between", open && "w-full !max-w-full lg:!p-[0_0_0_0_250px]")}>
                    <h2>{title}</h2>
                    <div className="flex items-center gap-2">
                        <Button variant="ghost" size="sm" className="cursor-pointer !bg-muted/10" onClick={onClick}>
