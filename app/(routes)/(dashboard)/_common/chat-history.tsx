...
const ChatHistory = () => {
    ...
    return (
        <div className={cn(`fixed top-0 left-0 h-full w-80 bg-white dark:bg-background border-r border-border z-[-9] transform transition-transform duration-300 ease-in-out`, open && isHistoryOpen ? "lg:left-64" : "left-0", isHistoryOpen ? "translate-x-0" : "-translate-x-full")}>
                ...
                {isPending ? (
                    ...
                    <ul className="w-full space-y-2.5 px-2 py-3">
                        {chatHistory?.map((chat) => (
                            <li key={chat.id}>
                                <Button className="w-full px-1 py-1.5 flex items-center gap02 rounded-lg hover:bg-muted cursor-pointer transition-colors"
                                ...
                                >
                                    ...
                                    <div className="w-full text-left">
                                        ...
                                        <p className="text-xs text-muted-foreground">
                                            {format(new Date(chat.createdAt), "MMM, d, yyy")}
                                        </p>
                                    </div>
                                </Button>
                            </li>
