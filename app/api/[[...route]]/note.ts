...
.patch("/update/:id", zValidator("param", noteIdSchema), zValidator("json", noteUpdateSchema),
getAuthUser, async (c) => {
    try {
        const user = c.get("user");
...
.delete("/delete/:id", zValidator("param", noteIdSchema), getAuthUser, async (c) => {
    try {
        const user = c.get("user");
        const { id } = c.req.valid("param");
        ...
        throw new HTTPException(500, { message: "Internal sercer error" });
    }
}
)
